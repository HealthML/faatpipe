...
rule assoc_missense_localcollapsing_eval_top_hits_h2:
    # calculates single-variant p-values and regression coefficients and other statistics for the most significant genes for missense variants
    input:
        h5_lof = expand(rules.export_plof_burden.output.h5, id = plinkfiles.getIds(), allow_missing=True),
        iid_lof = expand(rules.export_plof_burden.output.iid_txt,  id = plinkfiles.getIds(), allow_missing=True),
        ...
        seak_install = rules.install_seak.output
    output:
        out_ok = touch('work/association/sclrt_kernels_missense/{filter_highconfidence}/{pheno}/top_hits_h2/all.ok')
    params:
        kernels = ['linwcollapsed','linwcollapsed_cLOF','linwb','linwb_mrgLOF'], # genes with p < 1e-7 in one of these kernels will be analysed in detail
        phenotype = lambda wc: phenotypes[ wc.pheno ],
        covariate_column_names = config['covariate_column_names'],
        max_maf = config['maf_cutoff'],
        min_impact = config['min_impact'],
        out_dir_stats = lambda wc: 'work/association/sclrt_kernels_missense/{filter_highconfidence}/{pheno}/top_hits_h2/'.format(filter_highconfidence=wc.filter_highconfidence, pheno=wc.pheno),
        ids = plinkfiles.getIds(),
        filter_highconfidence = lambda wc: {'all': False, 'highconf_only': True}[wc.filter_highconfidence]
    resources:
        mem_mb=get_mem_mb(10000,1.5),
    ...
    threads:
        1
    log:
        'logs/association/sclrt_kernels_missense_eval_top_hits_h2/{filter_highconfidence}_{pheno}.log'
    conda:
        '../env/seak.yml'
    script:
        '../script/python/assoc_sclrt_kernels_missense_eval_top_hits_h2.py'
        
        
rule assoc_missense_localcollapsing_eval_top_hits_h2_all:
    # run above rule for all phenotypes - this rule will effectively run the entire pipeline for missense variants
    input:
        expand(rules.assoc_missense_localcollapsing_eval_top_hits_h2.output, pheno=phenotypes.keys(), filter_highconfidence=['all'])
...
rule assoc_spliceai_linw_eval_top_hits_h2:
    # calculates single-variant p-values and regression coefficients and other statistics for the most significant genes for splice variants
    input:
        h5_lof = expand(rules.export_plof_burden.output.h5, id = plinkfiles.getIds(), allow_missing=True),
        iid_lof = expand(rules.export_plof_burden.output.iid_txt,  id = plinkfiles.getIds(), allow_missing=True),
        ...
        seak_install = rules.install_seak.output
    output:
        touch('work/association/sclrt_kernels_spliceai/{filter_highconfidence}/{pheno}/top_hits_h2/all.ok')
    params:
        kernels = ['linb','linwb','lin','linw','linb_mrgLOF','linwb_mrgLOF','lin_cLOF','linw_cLOF'],
        phenotype = lambda wc: phenotypes[ wc.pheno ],
        covariate_column_names = config['covariate_column_names'],
        max_maf = config['maf_cutoff'],
        min_impact = config['splice_ai_min_impact'],
        out_dir_stats = lambda wc: 'work/association/sclrt_kernels_spliceai/{filter_highconfidence}/{pheno}/top_hits_h2/'.format(filter_highconfidence=wc.filter_highconfidence, pheno=wc.pheno),
        ids = plinkfiles.getIds(),
        filter_highconfidence = lambda wc: {'all': False, 'highconf_only': True}[wc.filter_highconfidence],
        debug = False
    log:
        'logs/association/sclrt_kernels_spliceai_eval_top_hits_h2/{filter_highconfidence}_{pheno}.log'
    resources:
        mem_mb=get_mem_mb(12000,1.5),
        time="6:00:00",
        partition="hpcpu"
    ...
    conda:
        '../env/seak.yml'
    script:
        '../script/python/assoc_sclrt_kernels_spliceai_eval_top_hits_h2.py'

rule assoc_spliceai_linw_eval_top_hits_h2_all:
    # runs rule above for all phenotypes - this rule will effectively run the entire pipeline for splice variants
    input:
         expand(rules.assoc_spliceai_linw_eval_top_hits_h2.output, pheno=phenotypes.keys(), filter_highconfidence=['all'])
...
rule assoc_deepripe_multiple_cholesky_eval_top_hits_h2:
    # calculates single-variant p-values and regression coefficients and other statistics for the most significant genes for DeepRiPe variants
    input:
        bed = expand(rules.link_genotypes.output.bed, id = plinkfiles.getIds()),
        h5_rbp_plus = expand(rules.run_deepripe_vep.output.h5, id = plinkfiles.getIds(), strand=['plus']),
        ...
        results_tsv = rules.assoc_deepripe_multiple_cholesky.output.results_tsv
    output:
        touch('work/association/sclrt_kernels_deepripe_multiple/{filter_highconfidence}/{pheno}/top_hits_h2/all.ok')
    params:
        kernels = ['lincholesky', 'linwcholesky', 'lincholesky_notLOF', 'linwcholesky_notLOF'],
        phenotype = lambda wc: phenotypes[ wc.pheno ],
        covariate_column_names = config['covariate_column_names'],
        max_maf = config['maf_cutoff'],
        min_impact = config['deepripe_min_impact'],
        out_dir_stats = lambda wc: 'work/association/sclrt_kernels_deepripe_multiple/{filter_highconfidence}/{pheno}/top_hits_h2/'.format(filter_highconfidence=wc.filter_highconfidence, pheno=wc.pheno),
        ids = plinkfiles.getIds(),
        filter_highconfidence = lambda wc: {'all': False, 'highconf_only': True}[wc.filter_highconfidence],
        rbp_of_interest = rules.assoc_deepripe_multiple_cholesky.params.rbp_of_interest,
        debug = False
    ...
    threads:
        1
    log:
        'logs/association/sclrt_kernels_deepripe_multiple_eval_top_hits_h2/{filter_highconfidence}_{pheno}.log'
    conda:
        '../env/seak.yml'
    script:
        '../script/python/assoc_sclrt_kernels_deepripe_multiple_eval_top_hits_h2.py'


rule assoc_deepripe_multiple_cholesky_eval_top_hits_h2_all:
    # runs rule above for all phenotypes - this will effectively run the entire pipeline for DeepRiPe variants
    input:
        expand(rules.assoc_deepripe_multiple_cholesky_eval_top_hits_h2.output, pheno=phenotypes.keys(), filter_highconfidence=['all'])
