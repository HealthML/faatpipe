...
for i, (chromosome, bed, mac_report, vep_tsv) in enumerate(geno_vep):
    ...
    for strand in ['plus', 'minus']:
        ...
        def test_gene(interval, seed):
            ...
            flag2 = -1
            if (pval_dict['pv_score_lincholesky'] < snakemake.params.sclrt_nominal_significance_cutoff) | (pval_dict['pv_score_linwcholesky'] < snakemake.params.sclrt_nominal_significance_cutoff):

                # do lrt tests
                call_lrt(GL, 'lincholesky')
                call_lrt(GWL, 'linwcholesky')
                ...
                if np.any(keep):
                    ...
                    call_score(GL, 'lincholesky_notLOF')
...
kern = ['lincholesky', 'linwcholesky', 'lincholesky_notLOF', 'linwcholesky_notLOF']
...
if snakemake.output.results_tsv.endswith('.gz'):
    with gzip.open(snakemake.output.results_tsv, 'wt') as outfile:
        outfile.write('#rbp: {}\n'.format(','.join(snakemake.params.rbp_of_interest)))
        pvals.to_csv(outfile, sep='\t', index=True, header=True)
else:
    with open(snakemake.output.results_tsv, 'w') as outfile:
        outfile.write('#rbp: {}\n'.format(','.join(snakemake.params.rbp_of_interest)))
        pvals.to_csv(outfile, sep='\t', index=True, header=True)
